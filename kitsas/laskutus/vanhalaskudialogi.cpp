    ...
    ennakkoModel_(new EnnakkoHyvitysModel(this))
{
    ...
    connect( ui->osoiteEdit, &QPlainTextEdit::textChanged, this, &VanhaLaskuDialogi::paivitaLaskutustavat);
    connect( ui->laskutusCombo, &QComboBox::currentTextChanged, this, &VanhaLaskuDialogi::laskutusTapaMuuttui);
    connect( ui->maksuCombo, &QComboBox::currentTextChanged, this, &VanhaLaskuDialogi::maksuTapaMuuttui);
...
//    connect( ui->hyvitaEnnakkoNappi, &QPushButton::clicked, [this] { EnnakkoHyvitysDialogi *dlg = new EnnakkoHyvitysDialogi(this, this->ennakkoModel_); dlg->show(); });
    ...
    connect( ui->mmMuistutusMaara, &KpEuroEdit::textChanged, this, &VanhaLaskuDialogi::paivitaSumma);
    connect( ui->mmViivastysCheck, &QCheckBox::clicked, this, &VanhaLaskuDialogi::paivitaSumma);
    connect( ui->mmViivastysAlkaa, &KpDateEdit::dateChanged, this, &VanhaLaskuDialogi::paivitaSumma);
    connect( ui->mmViivastysLoppuu, &KpDateEdit::dateChanged, this, &VanhaLaskuDialogi::paivitaSumma);
    ...
    connect( ui->asiakas, &AsiakasToimittajaValinta::valittu, this, &VanhaLaskuDialogi::asiakasValittu);
...
VanhaLaskuDialogi::~VanhaLaskuDialogi()
{
    kp()->settings()->setValue("LaskuDialogi", saveGeometry());
    delete ui;
}

void VanhaLaskuDialogi::paivitaSumma()
{
...
void VanhaLaskuDialogi::paivitaNapit()
{
...
void VanhaLaskuDialogi::tulosta(QPagedPaintDevice *printer) const
{
...
QString VanhaLaskuDialogi::otsikko() const
{
    return tr("Lasku");
}
...
void VanhaLaskuDialogi::lisaaTuote()
{
    TuoteDialogi* dialog = new TuoteDialogi(this);
    dialog->uusi();
}

void VanhaLaskuDialogi::tuotteidenKonteksiValikko(QPoint pos)
{
...
void VanhaLaskuDialogi::tulostaLasku()
{
...
void VanhaLaskuDialogi::asiakasValittu(int asiakasId)
{
...
void VanhaLaskuDialogi::taytaAsiakasTiedot(QVariant *data)
{
    ...
    if( id != asiakasId_) {
        ui->laskutusCombo->setCurrentIndex(ui->laskutusCombo->findData(map.value("laskutapa", VanhaLaskuDialogi::TULOSTETTAVA)));
        asiakasId_ = id;
    }
...
void VanhaLaskuDialogi::asiakasHaettuLadattaessa(QVariant *data)
{
    QVariantMap map = data->toMap();
    verkkolaskutettava_ =  kp()->asetukset()->luku("FinvoiceKaytossa") &&
            map.value("ovt").toString().length() > 9 &&
...
void VanhaLaskuDialogi::paivitaLaskutustavat()
{
...
void VanhaLaskuDialogi::laskutusTapaMuuttui()
{
...
void VanhaLaskuDialogi::maksuTapaMuuttui()
{
...
QVariantMap VanhaLaskuDialogi::data(QString otsikko) const
{
    QVariantMap map;
...
void VanhaLaskuDialogi::alustaRiviTab()
{
    ...
    connect( ui->tuoteView, &QTableView::customContextMenuRequested, this, &VanhaLaskuDialogi::tuotteidenKonteksiValikko);
...
QVariantMap VanhaLaskuDialogi::vastakirjaus(const QDate& pvm, const QString &otsikko) const
{
    TositeVienti vienti;
...
void VanhaLaskuDialogi::alustaMaksutavat()
{
...
qlonglong VanhaLaskuDialogi::laskeViivastysKorko() const
{
...
void VanhaLaskuDialogi::taydennaMaksumuistutuksenData(QVariantMap &map) const
{
...
void VanhaLaskuDialogi::naytaLoki()
{
...
void VanhaLaskuDialogi::tallenna(Tosite::Tila moodi)
{
...
void VanhaLaskuDialogi::tallennusValmis(QVariant *vastaus, bool toimita)
{
...
int VanhaLaskuDialogi::maksutapa() const
{
    return ui->maksuCombo->currentData().toInt();
}

void VanhaLaskuDialogi::ennakkoHyvitysData(int eraid, double eurot, QVariant *data)
{
    QVariantMap map = data->toMap();
    QVariantMap rivi;
...
void VanhaLaskuDialogi::alustaRyhmalasku()
{
    ...
    connect( ryhmalaskuTab_->model(),  &LaskutettavatModel::tallennettu, this, &VanhaLaskuDialogi::accept );
    connect( ryhmalaskuTab_->model(),  &LaskutettavatModel::tallennettu, this, &VanhaLaskuDialogi::tallennettuValmiina );
    connect( ryhmalaskuTab_->model(), &LaskutettavatModel::rowsInserted, this, &VanhaLaskuDialogi::paivitaNapit);
    setWindowTitle(tr("Ryhmälasku"));
}

void VanhaLaskuDialogi::lataa(const QVariantMap &map)
{
    ...
    connect( ui->lokiView, &QTableView::clicked, this, &VanhaLaskuDialogi::naytaLoki);
}

void VanhaLaskuDialogi::paivitaNakyvat()
{
...
void VanhaLaskuDialogi::lisaaEnnakkoHyvitys(int eraId, double eurot)
{
