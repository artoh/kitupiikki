...
QString KiertoWidget::kululaskuVirtuaalikoodi() const
{
    ...
    QString koodi = QString("4 %1 %2 000 %3 %4")
            .arg( iban_.valeitta().mid(2,16) )  // Tilinumeron numeerinen osuus
...
QByteArray KiertoWidget::kululaskuQr() const
{
    ...
    QString bic = iban_.bic();
    if( bic.isEmpty())
        return QByteArray();
    data.append(bic + "\n");
    data.append( tosite_->kumppaninimi() + "\n");
    data.append(iban_.valeitta() + "\n");
...
void KiertoWidget::lataaTosite()
{
    ...
    ui->ibanLabel->setText( iban_.valeilla() );
...
void KiertoWidget::paivitaViivakoodi(const QString& viite)
{
    ...
    if( iban_.isEmpty() || ( tosite_->tyyppi() != TositeTyyppi::KULULASKU && viite.isEmpty())) {
        ...
        ui->viivakoodiLabel->setText(LaskunAlaosa::code128(kululaskuVirtuaalikoodi()));
