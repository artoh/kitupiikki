...
void JsonKentta::set(const QString &avain, qlonglong arvo)
{
    if( map_.value(avain).toLongLong() != arvo )
    {
        map_[avain] = QVariant(arvo);
        muokattu_ = true;
    }
...
qlonglong JsonKentta::pitkaluku(QString &avain)
{
    return map_.value(avain).toLongLong();
}
