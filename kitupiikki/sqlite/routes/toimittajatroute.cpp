...
{
    QSqlQuery kysely(db());
    kysely.exec("select kumppani.id, kumppani.nimi, sum(summa.kredit) as summa, sum(avoin.sd) as avd, sum(avoin.sk) as avk, sum(vanha.sd) as vad, sum(vanha.sk) as vak from "
            "Kumppani JOIN ( select kredit, vienti.kumppani, vienti.id as vienti FROM "
